...
namespace SearchAlgorithms
{
    ...
    public static class UnityFrontierSearchWrapper
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        public static Tuple<List<T>, List<A>> FindShortestPath<T, A>(
            ...
            IEnumerable<A> actions,
            Func<T, A, Tuple<T, float>[]> successorFunction, // takes a node and an action, returns an array of tuples (successor, cost)
            HeuristicFrontier<T> frontier,
            SearchBenchmarkEntry diagnostics = null,
            bool verbose = false)
        {
            IEnumerable<T> initialNodes = new HashSet<T>().Append(initialNode);
            return FindShortestPathMultipleInitialNodes<T, A>(initialNodes, targetEvaluationFunction, actions, successorFunction, frontier, diagnostics, verbose);
        }
            ...
            bool verbose = false)
        {
            ...
            foreach (T initialNode in initialNodes)
            {
                frontier.Add(initialNode, 0);
                gScore[initialNode] = 0;
            }

            int nodeCount = 0;

            (double distance, T currentNode) = frontier.PopMinNode();
                ...
                foreach (A action in actions)
                {
                    ...
                    foreach (Tuple<T, float> neighbour_tuple in neighbour_tuples)
                    {
                        ...
                        if (totalCurrentDistance < gScore.GetValueOrDefault(neighbour, float.MaxValue))
                        {
                            ...
                            {
                                frontier.UpdateNodeScore(neighbour, totalCurrentDistance);
                            }
                            else
                            {
                                frontier.Add(neighbour, totalCurrentDistance);
                            }
                        }
                    }
                }
                (distance, currentNode) = frontier.PopMinNode();
            }
