...
static tl_Node	*can = ZN;

tl_Node *
right_linked(tl_Node *n)
{
    if (!n)
    {
        return n;
...
canonical(tl_Node *n)
{
    tl_Node *m;	/* assumes input is right_linked */
...
Canonical(tl_Node *n)
{
    tl_Node *m, *p, *k1, *k2, *prev, *dflt = ZN;
    int tok;
