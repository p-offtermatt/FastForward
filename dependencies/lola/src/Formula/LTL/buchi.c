...
BScc *scc_stack;
int accepting_state, bstate_count = 0, btrans_count = 0, rank;
...
{
    BTrans *s, *t;
    if (((a->final == accepting_state) || (b->final == accepting_state)) &&
            (a->final + b->final != 2 * accepting_state) &&
...
{
    if ((fin != accepting_state) && in_set(set, final[fin + 1]))
    {
...
void make_btrans(BState *s) /* creates all the transitions from a state */
{
    ...
    if (s->gstate->trans)
        ...
        {
            int fin = next_final(t->final, (s->final == accepting_state) ? 0 : s->final);
