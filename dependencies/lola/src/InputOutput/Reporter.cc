...
pthread_mutex_t Reporter::statusmutex = PTHREAD_MUTEX_INITIALIZER;
/*!
...
void ReporterSocket::abort(errorcode_t code) const
{
    ...
    if (errno != 0)
    {
        ...
        RT::data["call"]["error"] = strerror(errno);
    }
...
void ReporterStream::status(const char *format, ...) const
{
    ...
    // avoid mixture of several messages
    pthread_mutex_lock(&statusmutex);
    fprintf(stderr, "%s: ", markup(MARKUP_TOOL, PACKAGE).str());
    for(int i = 0; i < indentwidth; i++) fprintf(stderr," ");
